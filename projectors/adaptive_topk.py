import torch


class AdaptiveTopk:
    """
    This should be created for every parameter
    """
    def __init__(self, rank, verbose=False, update_proj_gap=200, scale=1.0, param_shape=None, beta=0.95):
        self.rank = rank
        self.verbose = verbose
        self.update_proj_gap = update_proj_gap
        self.scale = scale

        self.top_indices = None
        self.ema_beta = beta
        self.ema_indices = None
        self.reduced_dim = None
        assert param_shape is not None, "need param_shape to project back to original size"
        self.param_shape = param_shape

    def project(self, full_rank_grad: torch.Tensor, n_iter):
        if full_rank_grad.shape[0] >= full_rank_grad.shape[1]:
            # first update the ema indices
            self.update_ema_indices(full_rank_grad, 0)

            if self.top_indices is None or n_iter % self.update_proj_gap == 0:
                self.reduced_dim = 0
                self.top_indices = torch.topk(self.ema_indices, self.rank).indices
            low_rank_grad = full_rank_grad[self.top_indices, :]
        else:
            # first update the ema indices
            self.update_ema_indices(full_rank_grad, 1)

            if self.top_indices is None or n_iter % self.update_proj_gap == 0:
                self.reduced_dim = 1
                self.top_indices = torch.topk(self.ema_indices, self.rank).indices
            low_rank_grad = full_rank_grad[:, self.top_indices]
        return low_rank_grad

    def project_back(self, low_rank_grad: torch.Tensor):
        full_rank_grad = torch.zeros(self.param_shape, dtype=low_rank_grad.dtype, device=low_rank_grad.device)
        if self.reduced_dim == 0:
            full_rank_grad[self.top_indices, :] = low_rank_grad
        else:
            full_rank_grad[:, self.top_indices] = low_rank_grad
        return full_rank_grad * self.scale

    def update_ema_indices(self, full_rank_grad, dim):
        if self.ema_indices is None:
            self.ema_indices = torch.norm(full_rank_grad, dim=dim)
        else:
            self.ema_indices = self.ema_beta * self.ema_indices + (1 - self.ema_beta) * torch.norm(full_rank_grad, dim=dim)
